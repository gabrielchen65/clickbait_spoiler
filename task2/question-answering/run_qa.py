...
class DataTrainingArguments:
    ...
    evaluation_metric: Optional[str] = field(
        default="meteor", metadata={"help": "meteor or squad."}
    )
...
def main():
    ...
    metric = evaluate.load(data_args.evaluation_metric)

    def convertToMeteor(predictions, references):
        outp = ['']*len(predictions)
        outr = ['']*len(references)
        for i in range(len(predictions)):
            outp[i] = predictions[i]['prediction_text']
            outr[i] = references[i]['answers']['text']
        return outp, outr

    def compute_metrics(p: EvalPrediction):
        if data_args.evaluation_metric == "meteor":
            predictions, references = convertToMeteor(p.predictions, p.label_ids)
        else:
